// unfurl — fetch a page's metadata (OG/Twitter + oEmbed + favicon fallbacks) so
// the admin Links tab can cache a social link's title, description and image
// into `social_links.og_*`. The Socials act reads that cache and NEVER fetches
// at render time.
//
// PORT.SOC.7 (2026-08-02) — ported from the TitiLinks application
// (C:\dev\titilinks, supabase/functions/unfurl) essentially verbatim. Only the
// user-agent string and this header changed; the SSRF guard, the manual
// redirect walk, the caps and the layered parse are byte-for-byte the proven
// original. Do not "tidy" the guard: every branch of it is load-bearing.
//
